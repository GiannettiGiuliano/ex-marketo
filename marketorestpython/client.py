...
from .helper.exceptions import MarketoException
from .helper.http_lib import HttpLib
...
class MarketoClient:
    ...
    def _export_job_state_machine(self, entity, state, job_id):
        ...
        state_info = {'enqueue': {'suffix': '/enqueue.json', 'method': 'post', 'mode': 'nojson'}, 'cancel': {
            'suffix': '/cancel.json', 'method': 'post', 'mode': 'nojson'},
                      'status': {'suffix': '/status.json', 'method': 'get', 'mode': ''},
        ...
        result = self._api_call(
            ...
            self.host + '/bulk/v1/{}/export/{}{}'.format(entity, job_id, state_info[state]['suffix']), args,
            mode=state_info[state]['mode'])
        if state == 'file' and result.status_code is 200:
